...
rube <- function(model, data=NULL, inits=NULL, parameters.to.save=NULL,
    ...
    if (program=="jags") {
        ...
        if (!any(search() == "package:rjags")) stop("cannot load package 'R2jags'")
        ...
        if (!parallel) {
            ...
            rtn <- try(R2jags::jags(data=data, inits=thisInit,
            ...
            attach(data)
            rtn <- try(R2jags::jags.parallel(data=dataNames, inits=initsFun,
                                             parameters.to.save=parameters.to.save,
                                             model.file = modelFile,
                                             n.chains = n.chains, n.iter = n.iter,
                                             n.burnin = n.burnin, n.thin = n.thin,
