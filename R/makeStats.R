...
makeStats <- function(simsArr) {
    ...
    if (n.chains==1) {
        ...
        for (i in 1:n.varLong) {
            ...
            whole <- paste("mcmc.list(",
                           paste("mcmc(", pieces, ")", sep="", collapse=", "),
                           ")")
            temp <- try(coda::gelman.diag(eval(parse(text=whole)),
    ...
    varstats <- matrix(nrow = n.varLong, ncol = length(statnames),
                       dimnames = list(coda::varnames(codaForm), statnames))
    xtsvar <- matrix(nrow = n.chains, ncol = n.varLong)
    if (is.matrix(codaForm[[1]])) {
        for (i in 1:n.chains) {
            for (j in 1:n.varLong) {
                temp <- try(suppressWarnings(coda::spectrum0(codaForm[[i]][, j])), silent=TRUE)
        ...
        for (i in 1:n.chains) {
            temp <- try(suppressWarnings(coda::spectrum0(codaForm[[i]])), silent=TRUE)
            if (is(temp, "try-error")) {
                xtsvar[i,] <- NA
            } else {
                xtsvar[i,] <- temp$spec
    ...
    newEffSize <- coda::effectiveSize(codaForm)
